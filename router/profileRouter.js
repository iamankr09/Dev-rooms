...
router.get("/me", authenticate, async(request,response) => {
    try{
        ...
        if(!profile){
            return response.status(201).json({msg: "Profile not found"});
        }
...
router.post("/", authenticate, async(request, response) => {
    try{
        ...
        if(skills){
            profileObj.skills = skills.toString().split(",").map((skill) => skill.trim());
        }
        else profileObj.skills = "";
        if(bio) profileObj.bio = bio;
        else profileObj.bio = "";
        if(githubUserName) profileObj.githubUserName = githubUserName;
        else profileObj.githubUserName = "https://www.github.com/";
        ...
        // Insert to db
        let profile = new Proflie(profileObj);
        profile = await profile.save();
        profile.populate("user");
        if(image == "undefined" || image == "" || image == null){
            image = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTTOkHm3_mPQ5PPRvGtU6Si7FJg8DVDtZ47rw&usqp=CAU";
        }
...
router.delete("/experience/:expId", authenticate, async(request, response) => {
    try{
        ...
        if(removableIndex !== -1){
            ...
            return response.status(200).json({msg: "Experience is Deleted", profile: profile})
        } else {
            return response.status(404).json({msg: "Experience not found"});
        }
    } catch (error) {
        console.error(error);
        response.status(500).json({ errors: [{ msg: error.message }] });
